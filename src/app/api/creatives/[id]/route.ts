...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const q = await prisma.creativeQuestion.findUnique({
    where: { id },
  ...
  await prisma.$transaction([
    prisma.creativeProgress.deleteMany({ where: { questionId: id } }),
    prisma.creativeQuestion.delete({ where: { id } }),
  ]);
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (status === "NEW") {
    await prisma.creativeProgress.upsert({
    ...
    const existing = await prisma.creativeProgress.findUnique({ where: { userId_questionId: { userId, questionId: id } } });
    if (!existing) {
      await prisma.creativeProgress.create({ data: { userId, questionId: id, seenCount: 1, correctCount: 0, isLearned: false } });
    } else {
      await prisma.creativeProgress.update({ where: { userId_questionId: { userId, questionId: id } }, data: { isLearned: false } });
    }
  }
