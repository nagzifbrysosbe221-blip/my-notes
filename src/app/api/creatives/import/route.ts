...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!anyPrisma.creativeQuestion || typeof anyPrisma.creativeQuestion.create !== "function") {
      return new Response(
        "CreativeQuestion model not found in Prisma client. Run `npx prisma generate` and `npx prisma db push`, then restart the dev server.",
        { status: 500 }
      );
    }
    ...
    const created = await prisma.$transaction(
      toCreate.map((i) =>
        prisma.creativeQuestion.create({
          data: {
            subchapterId,
            prompt: i.question,
            answer: i.answer,
