...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Helpful guard if Prisma client wasn't regenerated
    const anyPrisma = prisma as any;
    if (!anyPrisma.shortQuestion || typeof anyPrisma.shortQuestion.create !== "function") {
      return new Response(
        "ShortQuestion model not found in Prisma client. Run `npx prisma generate` and `npx prisma db push`, then restart the dev server.",
        { status: 500 }
      );
    }
