...
export default function ScenarioPractice({ subId }: { subId: string }) {
  ...
  async function load() {
    ...
    try {
      ...
      const r = await fetch(`/api/scenarios/with-progress?${qs}`);
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (key === "r" || key === " ") {
        ...
        if (revealed) { e.preventDefault(); record(false); next(true); }
      } else if (key === "n" || key === "enter") {
        if (revealed) { e.preventDefault(); record(true); next(false); }
      }
    }
  ...
  async function record(correct: boolean) {
    ...
    if (!cur) return;
    try {
      await fetch("/api/scenarios/practice", {
        method: "POST",
      ...
      const qs = new URLSearchParams({ subchapterId: subId });
      if (concepts) qs.set("conceptTypes", concepts);
      const r2 = await fetch(`/api/scenarios/with-progress?${qs}`);
      if (r2.ok) {
          ...
          <div className="rounded border p-4">
            ...
            {revealed ? (
              <div className="mt-3 rounded bg-zinc-50 p-3 text-sm">
                <div className="mb-1 text-xs text-zinc-600">Suggested Answer</div>
